...
try:
    import numpy as np
    from numpy import nan as NaN, nan
except ImportError:
...
class Mavic (Robot):
    ...
    def fire_detection(self, verbose=True):
        ...
        # Prevent crash if no fire is found
        coord_fire = []
        ...
        try:
            ...
            if img is None:
                print("ERROR: Camera image is None")
                return []
            ...
            print(f"Fire ratio: {fire_ratio}%")

            if fire_ratio > 0.15:

                print("Smoke candidate detected")
                ...
                for i, c in enumerate(contours):
                    ...
                    print(
                        f"Contour {i}: center={centers[i]}, "
                        f"radius={radius[i]}"
                    )

                    # Keep only the biggest circle > 3 pixels
                    if radius[i] > 3 and radius[i] > radius_max:
                        ...
                        if verbose:
                            print(
                                f"fire detected, coordinates {coord_fire}"
                            )

                if verbose:

                    print("Saving fire_detection.jpg")
                ...
                print(f"Returning fire coordinate: {coord_fire}")
                return coord_fire

            else:
                print(
                    f"No fire detected. "
                    f"Fire ratio ({fire_ratio}%) below threshold."
                )
                return []

        except Exception as e:
            print("FIRE DETECTION CRASHED!")
            print("Error:", e)
            return []
        
        finally:
            print("===== FIRE DETECTION END =====\n")
    ...
    def run(self):
        ...
        while self.step(self.time_step) != -1:
            ...
            print(f"Current altitude: {altitude:.2f}")
